...
function WelcomePage(props) {
  ...
  async function registraion () {
    const data = await Registration(loginInp.current.value, passInp.current.value, nameInp.current.value);
    if(!data) return;
  }
    ...
    <div className={s.wrapper}>
      ...
      {isLoginForm ? (
        <div className={s.login_form}>
          ...
          <Input refer={loginInp} img={loginIcon} wrapclx={s.inp} text="Логин" />
          <Input refer={passInp} img={passwordIcon} wrapclx={s.inp} text="Пароль" />
        ...
        <div className={s.registration_form}>
          ...
          <Input refer={passInp} img={passwordIcon} wrapclx={s.inp} text="Пароль" />
          <Button click={registraion} classnames={s.btn} text="Зарегистрироваться" />
          <Button
