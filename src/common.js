...
export async function editPost(data, text, id) {
  const formData = new FormData();
  //Ограничение на картинку и песню ~28 МБ, а на видео ~90 МБ
  if(data.image !== '' && data.image.size < 30000000) formData.append('image', data.image); else formData.append('image', false);
  if(data.video !== '' && data.video.size < 100000000) formData.append('video', data.video); else formData.append('video', false);
  if(data.audio !== '' && data.audio.size < 30000000) formData.append('audio', data.audio); else formData.append('audio', false);
  fetch(
